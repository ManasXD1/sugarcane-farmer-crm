...
import { DeliveryService } from "@/lib/storage";
...
export default function AddDeliveryScreen() {
  ...
  const { farmers, refreshDeliveries } = useCRM();
  ...
  const handleSave = async () => {
    ...
    if (!form.quantityTons || isNaN(Number(form.quantityTons))) { Alert.alert("Error", "Valid quantity is required"); return; }
  ...
  return (
    <ScreenContainer edges={["top", "left", "right", "bottom"]}>
      ...
      <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.sectionCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            {form.farmerName ? (
              ...
              <View style={{ gap: 8 }}>
                <Text style={{ color: colors.muted, fontSize: 14 }}>Select a farmer from the list below:</Text>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.muted }]}>DELIVERY DETAILS</Text>
          <View style={[styles.sectionCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: colors.muted }]}>Quantity (tons) *</Text>
              <TextInput
                value={form.quantityTons}
                onChangeText={(v) => set("quantityTons", v)}
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: colors.muted }]}>Quality Grade</Text>
              <View style={styles.chipRow}>
                {GRADES.map((g) => (
                  <TouchableOpacity
                    key={g}
                    onPress={() => set("qualityGrade", g)}
                    activeOpacity={0.7}
                    style={[styles.chip, { backgroundColor: form.qualityGrade === g ? "#F59E0B" : colors.surface, borderColor: form.qualityGrade === g ? "#F59E0B" : colors.border }]}
                  >
                    <Text style={{ fontSize: 14, fontWeight: "700", color: form.qualityGrade === g ? "#fff" : colors.muted }}>Grade {g}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            <View style={styles.fieldGroup}>
              ...
              <TextInput
                ...
                placeholder="e.g. UP32-1234"
                placeholderTextColor={colors.muted}
                style={[styles.input, { backgroundColor: colors.background, borderColor: colors.border, color: colors.foreground }]}
              />
            </View>
            <View style={styles.fieldGroup}>
              <Text style={[styles.label, { color: colors.muted }]}>Received By</Text>
              <TextInput
                value={form.receivedBy}
                onChangeText={(v) => set("receivedBy", v)}
